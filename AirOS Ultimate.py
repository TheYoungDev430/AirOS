...
from tkinter import messagebox, Menu, Toplevel, Label, Button, Entry, scrolledtext, filedialog, Listbox, Scrollbar, Text, END, StringVar, OptionMenu
import subprocess
import os
import platform
import psutil
...
class AddUserPanel(Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Add User")
        self.geometry("300x200")

        Label(self, text="Username:", font=("Arial", 12)).pack(pady=5)
        self.username_entry = Entry(self, font=("Arial", 14))
        self.username_entry.pack(pady=5)

        Label(self, text="Password:", font=("Arial", 12)).pack(pady=5)
        self.password_entry = Entry(self, show="*", font=("Arial", 14))
        self.password_entry.pack(pady=5)

        Button(self, text="Create User", command=self.create_user).pack(pady=10)
...
class SkeepOS:
    ...
    def show_login_screen(self):
        ...
        self.root.bind("<Key>", self.show_user_selection)

    def update_time(self):
        self.time_label.config(text=strftime("%I:%M:%S %p\n%d-%b-%Y"))
        self.root.after(1000, self.update_time)

    def show_user_selection(self, event=None):
        self.root.unbind("<Key>")
        for widget in self.login_frame.winfo_children():
            widget.destroy()

        Label(self.login_frame, text="Select User:", fg="white", bg="black", font=("Arial", 16)).pack(pady=10)
        self.selected_user = StringVar(self.login_frame)
        user_list = list(self.users.keys())
        self.selected_user.set(user_list[0])
        OptionMenu(self.login_frame, self.selected_user, *user_list).pack(pady=5)

        Label(self.login_frame, text="Enter Password:", fg="white", bg="black", font=("Arial", 16)).pack(pady=10)
    ...
    def show_start_menu(self):
        ...
        menu.add_command(label="Add User", command=lambda: AddUserPanel(self.root))
...
class SettingsPanel(Toplevel):
    ...
    def save_password(self):
        ...
        if new_password:
            save_user(app.current_user, new_password)
